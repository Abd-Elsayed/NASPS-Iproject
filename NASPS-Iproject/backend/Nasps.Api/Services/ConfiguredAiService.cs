...
public sealed class ConfiguredAiService(HttpClient httpClient, IOptions<AiProviderOptions> options) : IAiService
{
    ...
    public Task<PersonalizedLearningPathDto> CreateLearningPathAsync(TraineeAiContextDto context, CancellationToken ct) =>
        CallAsync<PersonalizedLearningPathDto>("You are an evidence-based internship learning coach. Return valid JSON only.",
            $"Return summary, progressPercent, and 3-6 recommendations (order, title, description, reason, skills, difficulty, estimatedHours) for: {JsonSerializer.Serialize(context, JsonOptions)}", AiResponseValidator.Validate, ct);
