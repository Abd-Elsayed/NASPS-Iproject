...
export class AuthService {
    ...
    apiUrl = API_BASE_URL;
    adminEmail = 'admin@nasps.com';
    adminPasswordKey = 'nasps-admin-password';
    get adminPassword() { return localStorage.getItem(this.adminPasswordKey) ?? 'Admin@123'; }
    demoTraineeUsername = this.usernameFor(2);
    demoTraineePassword = this.temporaryPasswordFor(2);
    roleKey = 'nasps-role';
    currentEmailKey = 'nasps-current-trainee-email';
    pendingEmailKey = 'nasps-password-change-email';
    verifyEmailKey = 'nasps-verify-email';
    verifyEmailAddress = signal(localStorage.getItem(this.verifyEmailKey) ?? '', /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "verifyEmailAddress" }] : /* istanbul ignore next */ []));
    pendingLoginPassword = '';
    credentialsKey = 'nasps-trainee-credentials';
    tokenKey = 'nasps-access-token';
    userKey = 'nasps-current-user';
    token = signal(localStorage.getItem(this.tokenKey) ?? '', /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "token" }] : /* istanbul ignore next */ []));
    role = signal(this.readRole(), /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "role" }] : /* istanbul ignore next */ []));
    /** The signed-in user's own record (GET /api/auth/me), kept for the profile page and dashboards. */
    currentUser = signal(this.readStoredUser(), /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "currentUser" }] : /* istanbul ignore next */ []));
    profilePhotoUrl = signal('', /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "profilePhotoUrl" }] : /* istanbul ignore next */ []));
    ...
    identity = computed(() => {
        ...
        const roleName = user?.roleName ?? claims?.['roleName'];
        const department = user?.department ?? claims?.['department'] ?? '';
        const label = roleName === 'SuperAdmin' ? 'Super Admin' :
            roleName === 'Admin' ? (department ? `Admin · ${department}` : 'Admin') :
                roleName === 'Trainee' ? 'Trainee' :
                    this.role() === 'admin' ? 'Admin' : 'Trainee';
        return {
            name: user?.name?.trim() || claims?.['name']?.trim() || (this.role() === 'admin' ? 'Admin' : 'Trainee'),
            roleLabel: label,
            department,
    ...
    currentTraineeEmail = signal(localStorage.getItem(this.currentEmailKey) ?? 'sara@example.com', /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "currentTraineeEmail" }] : /* istanbul ignore next */ []));
    pendingPasswordEmail = signal(localStorage.getItem(this.pendingEmailKey) ?? '', /* @ts-ignore */
    ...(ngDevMode ? [{ debugName: "pendingPasswordEmail" }] : /* istanbul ignore next */ []));
    currentTraineeId = computed(() => {
    ...
    ...(ngDevMode ? [{ debugName: "currentTraineeId" }] : /* istanbul ignore next */ []));
    ...
    async loadCurrentUser() {
        if (!this.http || !this.token())
            return;
        try {
            const me = await firstValueFrom(this.http.get(`${this.apiUrl}/auth/me`));
    ...
    async loadProfilePhoto(expected = this.currentUser()?.hasProfilePhoto) {
    ...
    async updateCurrentUser(patch) {
        if (!this.http || !this.token())
            return false;
        try {
            const me = await firstValueFrom(this.http.put(`${this.apiUrl}/auth/me`, patch));
            this.currentUser.set(me);
            localStorage.setItem(this.userKey, JSON.stringify(me));
            return true;
        }
    ...
    async loginWithToken(identifierValue, password) {
        const identifier = identifierValue.trim();
        if (this.http) {
            try {
                const response = await firstValueFrom(this.http.post(`${this.apiUrl}/auth/login`, { identifier, password }));
                ...
                if (!role)
                    return { success: false, message: 'The access token does not contain a valid account role.' };
    ...
    async registerAdmin(payload) {
        ...
        try {
            return await firstValueFrom(this.http.post(`${this.apiUrl}/auth/register`, payload));
        }
        catch (error) {
            if (error instanceof HttpErrorResponse && error.status !== 0) {
    ...
    async verifyEmailCode(code) {
        ...
        if (!this.http || !email)
            return { success: false, message: 'Start from the sign-up or sign-in page.' };
        try {
            const result = await firstValueFrom(this.http.post(`${this.apiUrl}/auth/verify-email`, { email, code }));
    ...
    async resendVerificationCode() {
        ...
        if (!this.http || !email)
            return { success: false, message: 'Start from the sign-up or sign-in page.' };
        try {
            return await firstValueFrom(this.http.post(`${this.apiUrl}/auth/resend-verification`, { email }));
        }
        catch {
    ...
    async completePendingLogin() {
        ...
        this.clearEmailVerification();
        if (email && password)
            return this.loginWithToken(email, password);
        await this.router.navigate(['/login']);
        return { success: true, message: 'Email verified. Please sign in.' };
    }
    beginEmailVerification(email, password) {
    ...
    async changeFirstPasswordWithToken(password) {
        const email = this.pendingPasswordEmail() || localStorage.getItem(this.pendingEmailKey) || '';
        if (this.http && email && this.token()) {
            try {
                const result = await firstValueFrom(this.http.post(`${this.apiUrl}/auth/change-first-password`, { email, newPassword: password }));
                if (!result.success)
                    return result;
    ...
    async requestPasswordReset(emailValue) {
        ...
        if (this.http) {
            try {
                const result = await firstValueFrom(this.http.post(`${this.apiUrl}/auth/forgot-password/request`, { email }));
    ...
    async resetForgottenPassword(emailValue, code, password) {
        const email = emailValue.trim().toLowerCase();
        if (this.http) {
            try {
                return await firstValueFrom(this.http.post(`${this.apiUrl}/auth/forgot-password/reset`, { email, code, newPassword: password }));
            }
            catch (error) {
                if (error instanceof HttpErrorResponse && error.status !== 0) {
    ...
    login(role, identifierValue, password) {
        ...
        const trainee = this.data.trainees().find(item => item.email.toLowerCase() === identifier ||
            String(item.id) === identifier ||
            this.usernameFor(item.id).toLowerCase() === identifier);
        if (!trainee)
            return { success: false, message: 'No trainee account was found for this ID, username, or email.' };
        ...
        if (account.mustChangePassword && password === generatedPassword)
            account = { ...account, password: generatedPassword };
        if (account.password !== password)
            return { success: false, message: 'The trainee ID, username, or password is incorrect.' };
    ...
    changeFirstPassword(password) {
        const email = this.pendingPasswordEmail() || localStorage.getItem(this.pendingEmailKey) || '';
        if (!email)
            return { success: false, message: 'Sign in with your temporary password first.' };
        const accounts = this.readCredentials();
        const trainee = this.data.trainees().find(item => item.email.toLowerCase() === email);
        if (!trainee)
            return { success: false, message: 'The trainee account is no longer available.' };
    ...
    registerTrainee(traineeId, emailValue, previousEmail) {
        const email = emailValue.trim().toLowerCase();
        const accounts = this.readCredentials();
    ...
    usernameFor(traineeId) {
        return `NASPS-T${String(traineeId).padStart(3, '0')}`;
    }
    temporaryPasswordFor(traineeId) {
        return `NASPS@T${String(traineeId).padStart(3, '0')}`;
    }
    removeTrainee(emailValue) {
        const accounts = this.readCredentials();
        delete accounts[emailValue.trim().toLowerCase()];
        this.saveCredentials(accounts);
    }
    updateCurrentTraineeEmail(emailValue) {
        const email = emailValue.trim().toLowerCase();
        this.currentTraineeEmail.set(email);
        localStorage.setItem(this.currentEmailKey, email);
    }
    ...
    readRole() {
        ...
        if (storedToken)
            return this.roleFromToken(storedToken);
    ...
    normalizeCredential(account, traineeId, email) {
        return {
            traineeId,
    ...
    return new (__ngFactoryType__ || AuthService)(i0.ɵɵinject(i1.Router), i0.ɵɵinject(i2.DataService)); };
    static ɵprov = /*@__PURE__*/ i0.ɵɵdefineInjectable({ token: AuthService, factory: AuthService.ɵfac, providedIn: 'root' });
}
(() => { (typeof ngDevMode === "undefined" || ngDevMode) && i0.ɵsetClassMetadata(AuthService, [{
        type: Injectable,
        args: [{ providedIn: 'root' }]
    }], () => [{ type: i1.Router }, { type: i2.DataService }], null); })();
