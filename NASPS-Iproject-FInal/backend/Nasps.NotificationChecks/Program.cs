...
{
    private Fixture(NaspsDbContext db, CapturingEmailSender email,
        NotificationService service, User trainee, TrainingTask task)
    {
        Db = db; Email = email; Service = service; Trainee = trainee; Task = task;
    }
    ...
    {
        var db = new NaspsDbContext(new DbContextOptionsBuilder<NaspsDbContext>()
            .UseInMemoryDatabase($"nasps-checks-{Guid.NewGuid():N}").Options);
        ...
        return new Fixture(db, email, service, trainee, task);
    }
