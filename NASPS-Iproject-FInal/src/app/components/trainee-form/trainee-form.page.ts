...
@Component({
  ...
  imports: [ReactiveFormsModule, RouterLink, TraineeInsights],
...
export class TraineeFormPage {
  ...
  async save(): Promise<void> {
    ...
    if (this.id) {
      ...
      let access: TraineeAccess | null = null;
      try {
        access = await this.data.createTrainee({ ...value, status: 'Inactive' });
      } catch (error) {
        this.toast.show(error instanceof Error ? error.message : 'The trainee could not be created. Check the API connection and try again.', 'error');
        return;
      }
      if (!access) {
