...
export class DataService {
  ...
  /** Creates the trainee on the API (falls back to the local demo store when offline). */
  async createTrainee(trainee: Omit<Trainee, 'id'>): Promise<TraineeAccess | null> {
    ...
    const created = this.addTraineeLocal(trainee);
    return {
      traineeId: created.id,
      username: `NASPS-T${String(created.id).padStart(3, '0')}`,
      temporaryPassword: `NASPS@T${String(created.id).padStart(3, '0')}`,
    };
  }
  ...
  updateTrainee(id: number, patch: Partial<Trainee>): void {
    ...
    this.persist('nasps-trainees', this.trainees());
    const updated = this.trainees().find(item => item.id === id);
    if (updated) void this.updateTraineeOnServer(updated);
  }
  ...
  async addTask(task: Omit<TrainingTask, 'id'>): Promise<TrainingTask> {
    ...
    const created = { ...task, id: this.nextId(this.tasks()) } as TrainingTask;
    this.tasks.update(items => [created, ...items]);
    this.persist('nasps-tasks', this.tasks());
    return created;
  }

  async updateTask(id: number, patch: Partial<TrainingTask> & { clearAttachment?: boolean; clearSubmission?: boolean }): Promise<void> {
    this.tasks.update(items => items.map(item => {
      if (item.id !== id) return item;
      const next: TrainingTask = { ...item, ...patch };
      if (patch.clearAttachment) { next.attachment = undefined; next.attachmentType = undefined; next.attachmentSize = undefined; }
      if (patch.clearSubmission) { next.submission = undefined; next.submissionName = undefined; next.submissionType = undefined; next.submissionSize = undefined; }
      delete (next as Partial<typeof next> & { clearAttachment?: boolean }).clearAttachment;
      delete (next as Partial<typeof next> & { clearSubmission?: boolean }).clearSubmission;
      return next;
    }));
    this.persist('nasps-tasks', this.tasks());
    if (this.authorized) {
      try {
        await firstValueFrom(this.http!.put<TrainingTask>(`${this.apiUrl}/tasks/${id}`, patch));
      } catch {
        // demo store already updated
      }
    }
  ...
  private async updateTraineeOnServer(trainee: Trainee): Promise<void> {
    if (!this.authorized) return;
    try {
      await firstValueFrom(this.http!.put<void>(`${this.apiUrl}/trainees/${trainee.id}`, {
        name: trainee.name,
        email: trainee.email,
        internshipProgram: trainee.internshipProgram,
        phone: trainee.phone,
      ...
      // Local persistence keeps the demo usable while the ASP.NET API is offline.
    }
  }
}
