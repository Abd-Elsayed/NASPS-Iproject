...
export class AuthService {
  ...
  private readonly apiUrl = 'http://localhost:5222/api';
  ...
  /** Name + role label for the current session, decoded from the JWT (falls back to currentUser). */
  readonly identity = computed(() => {
    const claims = this.claimsFromToken(this.token());
    const user = this.currentUser();
    const roleName = (claims?.['roleName'] as string | undefined) ?? user?.roleName;
    const department = (claims?.['department'] as string | undefined) ?? user?.department ?? '';
    const label =
    ...
    return {
      name: (claims?.['name'] as string | undefined)?.trim() || user?.name || (this.role() === 'admin' ? 'Admin' : 'Trainee'),
      roleLabel: label,
      department,
  ...
  async loginWithToken(identifierValue: string, password: string): Promise<LoginResult> {
    ...
    if (this.http) {
      try {
        ...
        const role = this.roleFromToken(response.token) ?? response.role;
    ...
    const inferredRole: UserRole = identifier.toLowerCase() === this.adminEmail ? 'admin' : 'trainee';
    return this.login(inferredRole, identifier, password);
  }

  /// --- Admin self-registration + email verification (backend only) ---

  async registerAdmin(payload: { name: string; email: string; password: string; department: string }): Promise<RegisterAdminResult> {
    if (!this.http) return { success: false, message: 'Admin sign-up needs the API running.' };
    try {
      const result = await firstValueFrom(this.http.post<RegisterAdminResult>(`${this.apiUrl}/auth/register`, payload));
      if (result.success && result.email) this.beginEmailVerification(result.email, payload.password);
      return result;
    } catch (error) {
      if (error instanceof HttpErrorResponse && error.status !== 0) {
        return { success: false, message: error.error?.message ?? 'Sign-up failed.' };
      }
      return { success: false, message: 'Admin sign-up needs the API running.' };
    }
  }
  ...
  async verifyEmailCode(code: string): Promise<LoginResult> {
    ...
    try {
      const result = await firstValueFrom(
        this.http.post<{ success: boolean; message: string }>(`${this.apiUrl}/auth/verify-email`, { email, code }),
      );
      return result;
    } catch (error) {
      if (error instanceof HttpErrorResponse && error.status !== 0) {
  ...
  async changeFirstPasswordWithToken(password: string): Promise<LoginResult> {
    ...
    return this.changeFirstPassword(password);
  }
  ...
  async requestPasswordReset(emailValue: string): Promise<PasswordResetRequestResult> {
    ...
    const accountExists = email === this.adminEmail || this.data.trainees().some(item => item.email.toLowerCase() === email);
    if (!accountExists) return { success: false, message: 'No admin or trainee account was found for this email.' };
    const code = this.verification.requestCode('email', email);
    return { success: true, message: 'A password reset code was sent to your registered email.', demoCode: code };
  }
  ...
  async resetForgottenPassword(emailValue: string, code: string, password: string): Promise<LoginResult> {
    ...
    const verificationResult = this.verification.verifyCode('email', email, code);
    if (verificationResult !== 'verified') {
      return { success: false, message: verificationResult === 'expired' ? 'The reset code expired. Request a new one.' : 'The reset code is incorrect.' };
    }
    if (email === this.adminEmail) {
      localStorage.setItem(this.adminPasswordKey, password);
      return { success: true, message: 'The admin password was reset successfully. You can now sign in.' };
    }
    ...
    return { success: true, message: 'Your password was reset successfully. You can now sign in.' };
  }
