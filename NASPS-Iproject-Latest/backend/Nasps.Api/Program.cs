...
// Email + OTP. Real SMTP when Email:Host is set, otherwise the dev sender (logs + .eml files).
...
Console.WriteLine(emailOptions.HasSmtp
    ...
    : "[Email] Dev sender active (no Email:Host configured) - codes are logged and written to sent-emails/");
...
using (var scope = app.Services.CreateScope())
{
    ...
    if (!db.Users.Any())
    {
        ...
        foreach (var trainee in trainees)
        {
            ...
            trainee.EmailVerified = true; // demo trainees skip the OTP step
        }
